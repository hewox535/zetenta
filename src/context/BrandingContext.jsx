...
const DEFAULT = { name: 'zetenta', logoUrl: null, accent: null, businessId: null, ready: false };
...
export function BrandingProvider({ children }) {
  ...
  useEffect(() => {
    let active = true;
    supabase.rpc('get_branding', { p_host: window.location.hostname })
      .then(({ data }) => {
        if (!active) return;
        const b = Array.isArray(data) ? data[0] : data;
        if (b) {
          setBranding({
            name: b.name || 'zetenta', logoUrl: b.logo_url || null,
            accent: b.accent || null, businessId: b.business_id, ready: true,
          });
        } else {
          setBranding((prev) => ({ ...prev, ready: true }));
        }
      })
